...
let californiaFeature     = null;  // assigned in main()
let navStep               = -1;
...
const TOUR_YEAR_DESCS = {
  2018: "2018 was one of California's most destructive wildfire years to that point. The Camp Fire (November) destroyed the town of Paradise in Butte County and was the deadliest US wildfire in a century, while the Mendocino Complex (July–September) became the then-largest fire in California history at ~459,000 acres. Fire activity concentrated heavily along the northern Sierra Nevada and Coast Ranges. Land surface temperatures ran 1–2 °C above the 7-year mean across the Central Valley and Great Basin, reflecting the hot, dry summer that primed fuel conditions before the catastrophic fall fire weather.",
  2019: "2019 offered a relative reprieve for California, aided by a wetter than average winter that kept fuel moisture elevated through midsummer. Statewide acres burned were roughly 250,000, well below the decade average. The Kincade Fire (Sonoma County, October) and Walker Fire (Plumas County) were the most notable California events. The Pacific Northwest and Northern Rockies were comparatively more active. LST anomalies were mixed: the desert Southwest ran noticeably warm, while parts of the Pacific Coast and Great Basin stayed near or below their 7-year average, showing how year to year moisture variability shapes fire risk.",
  2020: "2020 was a historic and unprecedented year for western wildfire. California burned over 4.2 million acres, adding up to more than double any prior year in state history. The August Complex became the first California 'gigafire' (>1 million acres), joined by the SCU, LNU, and North complexes. Simultaneously, Colorado endured its two largest fires on record: the Cameron Peak (~208,000 acres) and East Troublesome (~193,000 acres). A powerful heat dome in mid-August drove Death Valley to 130 °F and generated broad, extreme LST anomalies across virtually the entire western US. Warm, dry surfaces lowered fuel moisture, and the fires themselves released additional heat into an already stressed atmosphere.",
  2021: "In late June 2021, a heat dome drove Portland, OR to 116 °F and Seattle, WA to 108 °F — 20–30 °F above normal — causing over 1,000 heat-related deaths across the Pacific Northwest. That same summer, California's Dixie Fire grew to ~963,000 acres, the largest single origin fire in state history, while Oregon's Bootleg Fire (~400,000 acres) generated its own pyrocumulonimbus weather. The Caldor Fire threatened South Lake Tahoe in August.",
  2022: "In 2022, California burned roughly 362,000 acres. The Mosquito Fire (Placer/El Dorado counties, ~76,000 acres) was the largest. LST anomalies were spatially heterogeneous: the Great Basin was cooler than average, while parts of the Pacific Coast and desert Southwest trended warmer.",
  2023: "In 2023, catastrophic wildfires across British Columbia and Alberta blanketed the northern and central US in smoke for weeks, attenuating some satellite FRP retrievals. Within the western US, fire activity was moderate: the Smith River Complex (Del Norte County, CA, ~100,000 acres) and scattered Oregon and Washington events were the most prominent. California's fire season was suppressed by a record-wet winter driven by an emerging El Niño. LST anomalies reflected this moisture: some northern areas showed near-average or below-average surface temperatures, while the southern desert Southwest maintained its persistent warm signal.",
  2024: "The Four Corners states saw above average fire activity in 2024. LST anomalies show that much of the Great Basin and California ran unusually cool, while southwestern and central states were much warmer.",
};
...
const TOUR_FRP_CONTEXT = {
  ...
  2019: "Despite the lower statewide totals, this region showed FRP signals elevated relative to its own multi year baseline.",
  2020: "This region produced some of the highest FRP values in the entire 2018–2024 dataset.",
  2021: "The Dixie Fire's FRP signature was notable for its duration: sustained high-intensity burning over weeks.",
  2022: "",
  2023: "",
  2024: "",
};
...
const TOUR_LST_CONTEXT = {
  2018: "",
  2019: "The desert Southwest's warm LST anomalies in 2019 stand out against an otherwise mixed national pattern, with persistent above average surface heat in the Four Corners states.",
  2020: "The 2020 LST pattern is dominated by the August heat dome, which produced the most intense positive anomalies in the dataset. Virtually the entire western US simultaneously exceeded its own already elevated 7-year averagee.",
  2021: "",
  2022: "",
  2023: "",
  2024: "",
...
const NAV_STEPS = [
  {
    ...
    targetId: 'layer-section',
    placement: 'left',
    body: 'Toggle <b>Fire Intensity (FRP)</b> and <b>LST Anomaly</b> on or off. The opacity sliders let you balance each layer — by default they\'re inversely linked (🔒): boosting one dims the other. Click 🔒 to decouple them.',
    onShow: _navAnimateOpacity,
  },
  {
    title: 'Year Slider',
    targetId: 'playback-section',
    placement: 'left',
    body: '...pretty self-explanatory.',
    onShow: _navAnimateYearSlider,
  },
  {
    title: 'Main Tour',
    targetId: 'play-btn',
  ...
  },
  {
    ...
    targetId: null,
    placement: 'map-ca',
    body: 'Double-click any highlighted state to zoom in and explore its data in detail. Double-click again to reset the view.',
    onNext: _navZoomCalifornia,
    nextDelay: 950,
  },
  {
    title: 'Selected Cell',
...
function _navAnimateOpacity() {
  ...
  function phase1() {
    s++;
    frpOpacity = +( 1.0 - (s / N) * 0.8 ).toFixed(2);
    lstOpacity = +Math.min(1, Math.max(0.2, 1.2 - frpOpacity)).toFixed(2);
    updateOpacitySliders(); setYear(currentYear, false);
    if (s < N) { navAnimTimer = setTimeout(phase1, 55); return; }
    navAnimTimer = setTimeout(() => {
      opacityLinked = false;
      if (lb) { lb.textContent = '🔓'; lb.classList.remove('locked'); }
      s = 0;
      function phase2() {
        s++;
        frpOpacity = +(0.2 + (s / N) * 0.6).toFixed(2);
        updateOpacitySliders(); setYear(currentYear, false);
        if (s < N) { navAnimTimer = setTimeout(phase2, 55); return; }
        navAnimTimer = setTimeout(() => {
          opacityLinked = true;
          if (lb) { lb.textContent = '🔒'; lb.classList.add('locked'); }
          frpOpacity = 1.00; lstOpacity = 0.20;
  ...
  navAnimTimer = setTimeout(phase1, 300);
}
...
function _navZoomCalifornia() {
  if (californiaFeature) zoomToFeature(californiaFeature);
}

function _navPinCACell() {
  if (!californiaFeature) return;
  const cell = findBestFRPCellInState(californiaFeature, currentYear);
  if (cell) pinCell(cell);
}

function _navEndCleanup() {
  clearPin();
  if (zoomedState) resetZoom();
}

function showNavModal() {
  document.getElementById('nav-tour-overlay').style.display = 'block';
  document.getElementById('nav-tour-modal').style.display   = 'block';
}

function hideNavModal() {
  document.getElementById('nav-tour-overlay').style.display = 'none';
  document.getElementById('nav-tour-modal').style.display   = 'none';
}

function _setNavTourActive(active) {
  playBtn.disabled = active;
  const navBtn = document.getElementById('nav-tour-btn');
  if (navBtn) navBtn.disabled = active;
}

function startNavTour() {
  _setNavTourActive(true);
  if (tourStep !== null) {
    tourStep = null;
    hideTourDesc();
    playBtn.textContent = '▶ Start Tour'; playBtn.classList.remove('tour-end');
  ...
  if (zoomedState) {
    zoomedState = null;
    if (statePaths) { statePaths.classed('state-dim', false); statePaths.classed('state-zoomed', false); }
...
function positionNavStep(step) {
  const popup = document.getElementById('nav-step-popup');
  delete popup.dataset.arrow;

  if (step.placement === 'map-ca' && californiaFeature && pathGen) {
    const [cx, cy] = pathGen.centroid(californiaFeature);
    const r  = mapArea.getBoundingClientRect();
    const pw = popup.offsetWidth || 340;
    const ph = popup.offsetHeight || 160;
    popup.style.left  = Math.max(8, r.left + cx - pw - 24) + 'px';
    popup.style.right = 'auto';
    popup.style.top   = Math.max(8, r.top + cy - ph / 2) + 'px';
    popup.dataset.arrow = 'right';
    return;
  }
  ...
  if (step.placement === 'left') {
    popup.style.right = (window.innerWidth - rect.left + 14) + 'px';
    popup.style.left  = 'auto';
    popup.style.top   = Math.max(8, Math.min(window.innerHeight - ph - 8,
      rect.top + rect.height / 2 - ph / 2)) + 'px';
    popup.dataset.arrow = 'right';
  }
}

function _highlightTarget(targetId) {
  document.querySelectorAll('.nav-highlight').forEach(el => el.classList.remove('nav-highlight'));
  if (targetId) { const el = document.getElementById(targetId); if (el) el.classList.add('nav-highlight'); }
}
...
function showNavStep(idx) {
  ...
  const popup = document.getElementById('nav-step-popup');
  const btn   = document.getElementById('nav-step-btn');
  popup.querySelector('.nav-step-title').textContent   = step.title;
  popup.querySelector('.nav-step-body').innerHTML      = step.body;
  popup.querySelector('.nav-step-counter').textContent = `${idx + 1} / ${NAV_STEPS.length}`;
  btn.innerHTML = step.btnText ?? 'Next &#9654;';
  btn.classList.toggle('tour-end', step.btnText === 'End');
  btn.disabled = false;
  popup.style.display = 'block';
  requestAnimationFrame(() => positionNavStep(step));
  _highlightTarget(step.targetId);
...
function advanceNavTour() {
  ...
  if (next >= NAV_STEPS.length) { endNavTour(); return; }
  if (step.nextDelay) {
    const btn = document.getElementById('nav-step-btn');
    if (btn) btn.disabled = true;
    setTimeout(() => showNavStep(next), step.nextDelay);
  } else {
    showNavStep(next);
  }
}
...
async function main() {
  ...
  californiaFeature = statesFeature.features.find(f => +f.id === 6) ?? null;
  ...
  // ── Nav tour controls ─────────────────────────────────────
  document.getElementById('nav-tour-btn').addEventListener('click', showNavModal);
  document.getElementById('nav-modal-start').addEventListener('click', () => {
    hideNavModal();
    localStorage.setItem('navTourSeen', '1');
    startNavTour();
  });
  document.getElementById('nav-modal-skip').addEventListener('click', () => {
    hideNavModal();
    localStorage.setItem('navTourSeen', '1');
  });
  document.getElementById('nav-step-btn').addEventListener('click', advanceNavTour);
  if (!localStorage.getItem('navTourSeen')) {
    setTimeout(showNavModal, 500);
  }
